...
enum TicketNewError {
    ...
    DescriptionError { error_message: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    let new_ticket = match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::DescriptionError{error_message }) => easy_ticket(
            title.clone(),
            "Description not provided".into(),
            status.clone(),
        ),
        Err(TicketNewError::TitleError { error_message }) => {
            panic!("{}", error_message)
        }
    };
    new_ticket
